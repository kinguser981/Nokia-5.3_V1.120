...
void resize_usage()
{
	...
	MSG(0, "  -V print the version number and exit\n");
	MSG(0, "  -r reserve sectors [default: 0]\n");//add by jiaozhiw: NHK_M528_A01-955 f2fs filesystem resize userdata 
	exit(1);
}
...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	if (!strcmp("fsck.f2fs", prog)) {
		...
		const char *option_string = "d:st:r:V"; //add by jiaozhiw: NHK_M528_A01-955 f2fs filesystem resize userdata 
		...
		while ((option = getopt(argc, argv, option_string)) != EOF) {
			...
			case 'V':
				show_version(prog);
				exit(0);
//add by jiaozhiw: NHK_M528_A01-955 f2fs filesystem resize userdata begin
			case 'r':
				if (strncmp(optarg, "0x", 2))
					ret = sscanf(optarg, "%"PRIu64"",
							&c.reserve_sectors);
					...
					ret = sscanf(optarg, "%"PRIx64"",
							&c.reserve_sectors);
 				break;
//add by jiaozhiw: NHK_M528_A01-955 f2fs filesystem resize userdata end
			default:
				err = EUNKNOWN_OPT;
				break;
			}
...
//add by jiaozhiw: NHK_M528_A01-955 f2fs filesystem resize userdata begin
	if (c.reserve_sectors)
		c.target_sectors = c.total_sectors - c.reserve_sectors;	
//add by jiaozhiw: NHK_M528_A01-955 f2fs filesystem resize userdata end
